...
flavor_map = {"ud-ud": "\pi", "ud-s": "K", "s-s": "\eta", "heavy-ud": "Hl", "heavy-s": "Hs", "heavy-heavy": "HH",
              "KPratio": "KPratio", "2k-pi": "2m_K-m_\pi", "Omega": "\Omega", 't0': 't_0', 'w0': "w_0",
              "fds_fd_ratio": "fds/fd", "fk_fpi_ratio": "fk/fpi"}
...
class data_params(ensemble_params):
    ...
    def __init__(self, filename):
        ...
        operator_choices = ("PP", "A4P", "PA4", "A4A4", "vectorave", "decayconst", "fsqrtm_linearhqet_continuum",
                            "fsqrtm_hqet_continuum", "fsqrtm_continuum")
        self.operator = next((i for i in operator_choices if i in filename), None)
        if self.operator is None:
            logging.warn("Operator type not determined from filename. If needed please at the"
                         "operator name to be parsed in data_paramters operator_choices")

    def __repr__(self):
        rstr = "{}_{}_{}_{}_{}_{}_{}_{}_{}".format(self.ename, self.beta, self.latsize, self.ud_mass, self.s_mass,
                                                   self.flavor_string, self.heavyness, self.smearing, self.operator)
...
class bootstrap_data(object):
    ...
    def __repr__(self):
        rstring = "{}: straps:{}, mean:{}, std:{}".format(repr(self.dp), self.dp.bootstraps, self.values.mass.mean(),
                                                          self.values.mass.std())
        return rstring
