...
from data_params import data_params, read_fit_mass, scale
...
def read_files(files, fitdata):
    ...
    for f in files:
        ...
        xvalue = xvalues(xtype, dp, fitdata)

        logging.info("read {} giving mpi={}, <observable>={}".format(dp, np.mean(xvalue), df.observable.mean()))
        #data[dp] = (scale[dp.beta], np.mean(xvalue), df.observable.values)
        data[dp] = (1.0/scale[dp.beta], np.mean(xvalue)*scale[dp.beta]**2, df.observable.values*scale[dp.beta])
...
def interpolate_mpisqr(data, phys_mpisqr):
    logging.info("Interpolating to chiral limit")
    ...
    for dp, d in data.iteritems():
        ...
        observables.append(np.mean(d[2]))
        variances.append(np.var(d[2]))
    ...
    def model_linear_mpisqr(phys_obs,A):
        return phys_obs*(1+A*(mpisqrs-phys_mpisqr) )

    def weighted_sqr_diff(phys_obs, A):
        sqr_diff = (observables - model(phys_obs, A))**2
        return np.sum(sqr_diff/variances)


    dof = float(len(observables)-2)

    phys_obs_params = {"phys_obs":guess_phys_obs, "error_phys_obs":guess_phys_obs*0.01}
    A_params = {"A":guess_A, "error_A":0.01}
    A_params = {"A":guess_A, "error_A":guess_A*0.01}
    # A_fixed = {"A":guess_A, "fix_A":True}
    A_fixed = {"A": 0.0, "fix_A":True}


    all_params= phys_obs_params.copy()
    model = model_linear_mpisqr
...
def interpolate(data, phys_mpisqr):
    ...
    def model_linear_both(phys_obs,A,M_pi):
        return phys_obs*(1+M_pi*(mpisqrs-phys_mpisqr)+A*a**2 )



    def weighted_sqr_diff(phys_obs, A, M_pi):
        sqr_diff = (observables - model(phys_obs, A, M_pi))**2
        return np.sum(sqr_diff/variances)
    ...
    phys_obs_params = {"phys_obs":guess_phys_obs, "error_phys_obs":guess_phys_obs*0.01}
    A_params = {"A":guess_A, "error_A":0.01}
    M_pi_params = {"A":guess_M_pi, "error_A":guess_M_pi*0.01}
    # A_fixed = {"A":guess_A, "fix_A":True}
    # A_fixed = {"A": 0.0, "fix_A":True}
...
def interpolate_chiral_spacing(options):
    ...
    fr = interpolate_mpisqr(alldata, phys_mpisqr)
    print fr

    fit_results = {"all": fr}
    for g in groups:
        fr = interpolate_mpisqr(g, phys_mpisqr)
        fit_results[g.keys()[0].beta] = fr

    for b,i in fit_results.iteritems():
        print b
        print i.values
    exit()
