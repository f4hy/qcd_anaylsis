...
print "# alpha_s (1, 2, 3, 4 loop at mMS) = $alpha1 $alpha2 $alpha3 $alpha4\n";
...
$Zm2 = $Zm1 + (6.248*$beta0-3.739)*$as**2;
$Zm3 = $Zm2 + (23.497*$beta0**2+6.248*$beta1+1.019*$beta0-29.94)*$as**3;
print "# Zm to pole mass (1,2,3 loop) = $Zm1  $Zm2  $Zm3\n";
...
$alpha4 = $alpha3 + $pi/($beta0*$L) * 1/($beta0**3*$L**3)*(
    $beta1**3/$beta0**3 * (-(log($L))**3 + 5/2*(log($L))**2 + 2*log($L) - 1/2)
    -3*$beta1*$beta2/$beta0**2 * log($L) + $beta3/(2*$beta0) );
print "# alpha_s (1, 2, 3, 4 loop at mpole) = $alpha1 $alpha2 $alpha3 $alpha4\n";
...
$CG = - 177/64 - 5/72*$pi**2 - 1/18*$pi**2*$ln2 - 11/36*$zeta3
    + (-79/144 - 7/108*$pi**2)*$L + 13/16*$L**2;
$CH = 727/432 - 1/6*$pi**2;
$CL = 47/288 + 1/36*$pi**2 + 5/72*$L - 1/24*$L**2;
$CM = 0;
...
$CGG = -62575/62208 - 231253/46656*$pi**2 - 517/324*$pi**2*$ln2
    ...
    + (115/54 - 121/648*$pi**2 + 1/36*$pi**2*$ln2 + 37/48*$zeta3 
       - 95/1944*$pi**4)*$L + (2257/576 + 91/432*$pi**2)*$L**2 - 13/8*$L**3;
...
$CGL = 24457/46656 + 5575/8748*$pi**2 + 19/324*$pi**2*$ln2 - 1/81*$pi**2*$ln2**2
    ...
    - 1/162*$ln2**4 - 4/27*$a4 + (-319/5184 + 11/972*$pi**2 + 83/216*$zeta3)*$L
    + (-469/864 - 7/648*$pi**2)*$L**2 + 25/144*$L**3;
...
$Cmu2 = $Cmu1 + $as**2*$C2;
$Cmu3 = $Cmu2 + $as**3*$C3;
print "C(mu)  1,2,3-loop = $Cmu1  $Cmu2  $Cmu3\n";
