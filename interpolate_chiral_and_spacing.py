...
def read_files(files, fitdata):
    ...
    for f in files:
        ...
        hbar_c = 197.327
        data[dp] = (hbar_c/scale[dp.beta], np.mean(xvalue)*scale[dp.beta]**2, df.observable.values*scale[dp.beta])
    return data
...
def interpolate(data, phys_mpisqr):
    ...
    def model_linear_both(phys_obs,A,M_pi):
        return phys_obs+M_pi*(mpisqrs-phys_mpisqr)+A*(a**2)
